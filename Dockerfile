...
ARG TFENV_VERSION=choose-tfenv-dir
...
RUN apk update && \
    ...
    curl https://releases.hashicorp.com/vault/${VAULT_VERSION}/vault_${VAULT_VERSION}_linux_amd64.zip --output - | \
      unzip -d /usr/bin/ - && \
      chmod +x /usr/bin/vault && \
    git clone --single-branch --branch ${TFENV_VERSION} \
